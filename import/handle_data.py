...
# Pull some important values out of each line of the master db - we want to associate POS IDs
# from the old database with new owher accounts, so make a dict with the email as the key and POS ID as the value
def process_master_db_data(master_db_raw_data):
    master_db_dict = {}
    for rd in master_db_raw_data:
        master_db_dict[normalize_email(rd[5])] = rd[1]
...
def get_pos_id_by_email(master_db_data, email):
    return master_db_data[email] if email in master_db_data and master_db_data[email] != '' else None
